...
describe('isValidContractId', () => {
  it('validates contract IDs', () => {
    expect(isValidContractId(`${VALID_ADDRESSES.mainnetSingleSig}.my-contract`)).toBe(
      true
    );
    expect(isValidContractId(`${VALID_ADDRESSES.testnetSingleSig}.test-contract-123`)).toBe(
      true
    );
  });
  ...
  it('rejects invalid contract IDs', () => {
    ...
    expect(isValidContractId(`${VALID_ADDRESSES.mainnetSingleSig}.1-invalid`)).toBe(false);
  });
});
...
describe('isValidTxId', () => {
  it('validates transaction IDs', () => {
    expect(
      isValidTxId('0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef')
    ).toBe(true);
  });
...
describe('shortenAddress', () => {
  it('shortens address with default length', () => {
    expect(shortenAddress(VALID_ADDRESSES.mainnetSingleSig)).toBe('SPJ6H...N5NT4');
  });

  it('shortens address with custom length', () => {
    expect(shortenAddress(VALID_ADDRESSES.mainnetSingleSig, 3)).toBe('SPJ...NT4');
  });
...
describe('getNetworkFromAddress', () => {
  it('identifies mainnet addresses', () => {
    expect(getNetworkFromAddress(VALID_ADDRESSES.mainnetSingleSig)).toBe('mainnet');
    expect(getNetworkFromAddress(VALID_ADDRESSES.mainnetMultiSig)).toBe('mainnet');
  });

  it('identifies testnet addresses', () => {
    expect(getNetworkFromAddress(VALID_ADDRESSES.testnetSingleSig)).toBe('testnet');
    expect(getNetworkFromAddress(VALID_ADDRESSES.testnetMultiSig)).toBe('testnet');
  });
...
describe('isAddressForNetwork', () => {
  it('matches exact networks and treats devnet as testnet address space', () => {
    expect(isAddressForNetwork(VALID_ADDRESSES.mainnetSingleSig, 'mainnet')).toBe(
      true
    );
    expect(isAddressForNetwork(VALID_ADDRESSES.testnetSingleSig, 'testnet')).toBe(
      true
    );
    expect(isAddressForNetwork(VALID_ADDRESSES.testnetSingleSig, 'devnet')).toBe(
      true
    );
    expect(isAddressForNetwork(VALID_ADDRESSES.mainnetSingleSig, 'testnet')).toBe(
      false
    );
  });
});
...
describe('parseContractId', () => {
  it('parses valid contract IDs', () => {
    const result = parseContractId(`${VALID_ADDRESSES.mainnetSingleSig}.my-contract`);
...
describe('createContractId', () => {
  it('creates contract IDs', () => {
    expect(createContractId(VALID_ADDRESSES.mainnetSingleSig, 'my-contract')).toBe(
